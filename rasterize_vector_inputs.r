...
outfolder <- "C:/Users/Tyler/Google Drive/NFWF Cross Realm/Data/Rasterized Inputs NAD83 UTM/"  # folder for rasterized outputs
template30 <- raster("C:/Users/Tyler/Google Drive/NFWF Cross Realm/Data/Rasterized Inputs NAD83 UTM/HUC12_zones_30m_NAD83UTM.tif")  # template to which new rasters should be snapped
...
### USER INPUTS
infile <- "C:/Work/SpatialData/NFWF_Cross_Realm/NHD_streams_MHB_NAD83UTM.shp"  # name of shapefile to convert
...
outfilename <- "streams.tif"   # what you want to call the output file
...
writeRaster(outraster, paste0(outfolder, outfilename), format="GTiff", prj=TRUE, overwrite=TRUE)

# or use fasterize (for polygons)
vector.sf <- st_read(infile)   # read in shapefile
vector.sf <- st_transform(vector.sf, proj4string(template30))# reproject shapefile to match template 
outraster <- fasterize(sf=vector.sf, raster=template30, field=NULL, fun=functionname)
