...
#                                                                   #
#                                                                   #
#   TO RUN THIS TOOL, HIT 'Run Document' NEXT TO THE GREEN ARROW    #
#                  IN THE HEADER OF THIS TAB.                       #
#                                                                   #
#                                                                   #
#   (There is no need to understand or interact with the code       #
    ...
    uiOutput("wt.opt")
      ...
      output$wt.opt <- renderUI({
        ...
        lapply(1:length(targets), function(i) {
            if(length(input$action.sel)==0) {output$wt.opt <- renderText({" "})} else {
    ...
    uiOutput("dirconn.opt")
      ...
      observeEvent(input$action.sel,{
        output$dirconn.opt <-renderUI({
          print(sum(targets %in% c("Stream health","Normative flow regime")))
          if(sum(targets %in% c("Stream health","Normative flow regime"))==0) {
            output$dirconn.opt <- renderText({" "})} else {
            selectInput("dirconn.sel", "Consider downstream benefits for stream target(s)?",
                    c("No" = "1",
                     "Yes (small benefits)" = "2",
                     "Yes (large benefits)" = "3")
                    )
            } 
        })
      })
...
```{r}
    ...
    observeEvent(input$action.sel, ({
        ...
        #maskLyr<-raster(paste0("runs/data/ActionMasks/",act.lyrs[act.lyrs$label==input$action.sel,"masklyr"]))
        targCond<-list()
        ...
        leafletProxy("map") %>%
            ...
            #addRasterImage(maskLyr, colors=pal, opacity=0.9, group="Action Footprint", maxBytes=6000) %>%
    ...
    observeEvent(input$check.stat,{
      ...
      if (isTRUE(file.exists(outputFile))) {
        ...
        leafletProxy("map") %>%
          clearControls() %>%
          addTiles() %>%
          addRasterImage(result, colors=rpal, opacity=0.9, group="Result", maxBytes=6000) %>%
...
```{r}
    ...
    observeEvent(input$save.settings,{
      ...
      dat<-list("Settings" = list("removal rule"=2, "edge removal"=1, 
                    ...
                    "planning unit layer file"=paste0("runs/data/ZonationData/HUC12_zones_90m_NAD83UTM.tif"),  
                    "use cost"=0, 
                    "use mask"=0, 
