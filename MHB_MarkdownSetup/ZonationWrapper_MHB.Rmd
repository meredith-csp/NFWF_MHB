...
observeEvent(input$load.inputs,{   
  ...
  observeEvent(input$save.settings,{ 
    ...
    potential_wts<-c(input$targ1,input$targ2,input$targ3,input$targ4,input$targ5,input$targ6,input$targ7)
    input_raster_dir<-paste0("runs/data/TargetCondition/",unique(targ.summ[targ.summ[,input$action.sel]==1,"dir"]))   
    
    create_zproject_rev(name="runs",dir=".",variants=variant_name,
                    spp_template_dir=input_raster_dir,spp_file_pattern="*.tif",   
                    overwrite=T,
                    weight=potential_wts[1:length(targ.summ[targ.summ[,input$action.sel]==1,"name"])])  #not aligning explicitly with specific targets
  ...
  observeEvent(input$check.stat,{
    ...
    output<-list.files(path = paste0("./runs/",variant_name), pattern = "wrscr.compressed.tif")
    if (length(output)>0) {
